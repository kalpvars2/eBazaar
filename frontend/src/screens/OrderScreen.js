						...
						<ListGroup variant='flush'>
							...
							{!order.isPaid && (
								<ListGroup.Item>
									{loadingPay && <Loader />}
									{!sdkReady && <Loader />}
									{!userInfo?.isAdmin && <PayPalButton amount={order.totalPrice} onSuccess={successPaymentHandler}/>}
								</ListGroup.Item>
							)}
							{loadingDeliver && <Loader />}
							{
								userInfo?.isAdmin && order.isPaid && !order.isDelivered && (
									<ListGroup.Item>
										<Button className='btn btn-sm w-100' type='button' onClick={deliverHandler}>
											Mark As Delivered
										</Button>
									</ListGroup.Item>
								)
